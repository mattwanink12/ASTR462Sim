!!****if* source/Simulation/SimulationMain/SupersonicHydroJets/Grid_bcApplyToRegionSpecialized
!!
!!
!! NAME
...
jj=j+yOffset
                 !do j=endPoints(LOW,JAXIS),endPoints(HIGH,JAXIS)   !y-direction
radius = abs(yCoord(jj) - sim_jetYCtr)
!!$if ((i==1) .and. (ivar == DENS_VAR)) then
!!$print*,j,yCoord(jj),jj,(radius <= 0.04),blockHandle
!!$!print*,secondDir-JAXIS
!!$endif
                    ...
                    if (radius <= 0.05) then
                       ...
                       if (gridDataStruct==CENTER) then
                          ...
                          if ((ivar == ENER_VAR) .or. (ivar==EINT_VAR)) then
                             ...
                             tmp_B2 = 0. !0.5*(sim_bxAmbient**2 + sim_byAmbient**2)
                          ...
                          if (ivar == MAGZ_VAR) regionData(i,j,1:ke,ivar)= 0.
